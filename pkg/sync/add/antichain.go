...
// Units TODO
func Units(dag gomel.Dag, adder gomel.Adder, preunits [][]gomel.Preunit, fallback gsync.QueryServer, where string, log zerolog.Logger) error {
	return nil
}

// Unit adds a preunit to the dag and returns an error if it fails.
func Unit(dag gomel.Dag, adder gomel.Adder, preunit gomel.Preunit, fallback gsync.QueryServer, where string, log zerolog.Logger) error {
...
func Antichain(dag gomel.Dag, adder gomel.Adder, preunits []gomel.Preunit, fallback gsync.QueryServer, where string, log zerolog.Logger) *gomel.AggregateError {
	//var wg sync.WaitGroup
