...
func NewFactory(pid uint16, tcoin *tcoin.ThresholdCoin, shareProvider map[uint16]bool) gomel.RandomSourceFactory {
	return &coinFactory{
		pid:           pid,
		tc:            tcoin,
...
func newCoin(pid uint16, dag gomel.Dag, tcoin *tcoin.ThresholdCoin, shareProvider map[uint16]bool) gomel.RandomSource {
	c := &coin{
		pid:           pid,
		dag:           dag,
		tc:            tcoin,
...
// NewSeededCoinFactory returns a coin factory based on a fixed ThresholdCoin generated by the provided seed.
...
func NewSeededCoinFactory(nProc, pid uint16, seed int) gomel.RandomSourceFactory {
	...
	return NewFactory(pid, myTC, shareProviders)
}
...
func (c *coin) checkCompliance(u gomel.Unit, _ gomel.Dag) error {
	...
	if gomel.Prime(u) && !gomel.Dealing(u) {
		...
		if rb := c.randomBytes.Get(u.Level() - 1); rb != nil {
			...
			if !c.tc.VerifyCoin(coin, nonce(u.Level()-1, u.EpochID())) {
				return errors.New("incorrect random bytes")
			}
		}
...
func (c *coin) combineShares(level int) ([]byte, error) {
	...
	if !c.tc.VerifyCoin(coin, nonce(level, c.dag.EpochID())) {
