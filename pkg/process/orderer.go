...
func NewOrderer(linearOrdering gomel.LinearOrdering, attemptTimingRequests <-chan struct{}, orderedUnits chan<- gomel.Unit) *Orderer {
	return &Orderer{
...
func (o *Orderer) extendOrder() {
	for round := range o.extendOrderRequests {
		units := o.linearOrdering.TimingRound(round)
		for _, u := range units {
			o.orderedUnits <- u
		}
	}
